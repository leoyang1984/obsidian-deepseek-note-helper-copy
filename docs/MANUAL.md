...
**What it is:**
Don't want to open the sidebar? No problem. Select any text in your editor, press `Cmd+Shift+J` (Mac) or `Ctrl+Shift+J` (Win), and a sleek, draggable floating window pops up directly over your text. The AI answers right under your fingertips via real-time streaming, and the text you selected stays pinned at the top of the window so you never lose context.
...
## ✨ 7. “耳边低语”：划词浮窗 AI Chat
**是什么：**
不想打开侧边栏？没问题。在编辑器里随便选中一段文本，按下 `Cmd+Shift+J` (Mac) 或者 `Ctrl+Shift+J` (Win)，一个优雅、支持拖拽和拉伸的悬浮窗口就会直接弹在你的光标旁边。AI 通过极速的流式输出在你的指尖解答疑问。被选中的文本会被吸附在浮窗顶部，让你在 Obsidian 编辑器高亮消失时也能随时查看上下文。
