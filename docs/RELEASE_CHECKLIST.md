...
## ⚪ 0. 版本判断与判定 (Version Strategy)
在动手改任何配置前，先确定本次更新的性质并进行审计：
- [ ] **审计当前版本**: 确认 GitHub 上已发布的最新版本号，确保本次更新**高于**当前线上版本。
- [ ] **Major (主版本)**: 包含不兼容的 API 变更或底层架构重写（如从 V2 到 V3）。
- [ ] **Minor (次版本)**: 增加了向下兼容的新功能（如本次 v1.8.0 的全能 UI 联动）。
- [ ] **Patch (补丁版)**: 仅包含向下兼容的问题修复（如代码优化、小 Bug 修复）。
...
- [ ] **提取发行版文件**: 脚手架会在根目录下生成类似于 `releases/v1.8.0/` 的文件夹。
- [ ] **生成发布文案 (Release Notes)**: 在该文件夹内创建一个 `release-notes.md`，从 `README.md` 中提取本次更新日志，方便发布时直接复制粘贴。
